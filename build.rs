...
fn main() {
    ...
    // (layout → memory → device → symbols). Downstream binaries opt in with one
    // `-Tws63-link.x` (see ws63-examples/blinky/build.rs).
    ...
    // riscv-rt symbol defaults come last.
    fs::write(
        &link_out,
    ...
    .expect("Failed to write ws63-link.x");
